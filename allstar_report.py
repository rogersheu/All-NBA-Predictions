...
def generate_report():
    df_modeled = pd.read_csv(fr".\data\dailystats\{curr_dt}\stats_{curr_dt_no_delim}_modeled.csv")
    df_genstat = pd.read_csv(fr".\data\dailystats\{curr_dt}\totals_{curr_dt_no_delim}.csv")
    ...
    team_to_conference = {team: conference for conference, team_list in conference_to_teams.items() for team in team_list}
    ...
    pos_to_frontback = {pos: frontback for frontback, position_list in frontcourt_backcourt.items() for pos in position_list}
...
def make_teams(df: pd.DataFrame):
    ...
    for player in allstar_dict:
        for conference in ["east", "west"]:
            team_size[conference] = len(teams[conference]["frontcourt"]) + len(teams[conference]["backcourt"])
            replacement_count[conference] = len(injury_replacements[conference]["frontcourt"]) + len(injury_replacements[conference]["backcourt"])
        
        if (team_size["west"] + replacement_count["west"]) == (TOTAL_PER_TEAM + EXTRA_FOR_INJURY) and (team_size["east"] + replacement_count["east"]) == (TOTAL_PER_TEAM + EXTRA_FOR_INJURY):
            break
        name = player["Player"]
        conference = player["Conf"]
        frontback = player["FrontBack"]
        avg = player["Avg"]
        if len(teams[conference]["frontcourt"]) + len(teams[conference]["backcourt"]) < TOTAL_PER_TEAM \
        and len(teams[conference][frontback]) < MAX_POS[frontback]:
            teams[conference][frontback].append((name, avg))
        elif len(injury_replacements[conference]["frontcourt"]) + len(injury_replacements[conference]["backcourt"]) < (EXTRA_FOR_INJURY):
